...
const aboutData: Record<Locale, AboutData> = {
  ko: {
    paragraphs: [
      ...
      '또한 <span class="text-[var(--color-text)] font-medium">AI 도구를 활용한 개발 생산성 극대화</span>를 추구합니다. Claude Code, Codex, GitHub Copilot 등을 활용해 탐색·문서화·구현 속도를 높이고, 실제 사용과 운영 조건으로 결과를 검증합니다. AI를 만드는 것뿐 아니라, AI와 함께 일하는 방식에도 전문성을 갖추고 있습니다.',
    ],
    tags: [
  ...
  en: {
    paragraphs: [
      ...
      'Also pursues <span class="text-[var(--color-text)] font-medium">maximizing development productivity with AI tools</span>. Uses Claude Code, Codex, and GitHub Copilot to accelerate exploration, documentation, and implementation, then validates results against real use and operational constraints. Experienced not only in building AI, but also in working with AI.',
    ],
    tags: [
      "LLM / Agent", "Computer Vision", "Speech (STT/TTS)", "Search System",
      "Recommender", "MCP", "Unreal Engine", "MLOps", "PyTorch", "Azure ML", "FastAPI",
  ...
  ja: {
    paragraphs: [
      ...
      'また、<span class="text-[var(--color-text)] font-medium">AIツールを活用した開発生産性の最大化</span>を追求しています。Claude Code、Codex、GitHub Copilotなどで探索・文書化・実装を加速し、実際の利用と運用条件で結果を検証します。AIを作るだけでなく、AIと共に働く方法にも専門性を持っています。',
    ],
    tags: [
      "LLM / Agent", "Computer Vision", "Speech (STT/TTS)", "Search System",
      "Recommender", "MCP", "Unreal Engine", "MLOps", "PyTorch", "Azure ML", "FastAPI",
